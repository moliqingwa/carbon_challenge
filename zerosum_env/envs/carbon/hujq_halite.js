// Copyright 2020 Kaggle Inc
      ...
      async function renderer({
        ...
        const colors = {
          bg: "#000B49",
          bgGradientTop: "#1787f5",
          bgGradientBottom: "#aa9dc9",
      ...
      `;
        ...
        if (!parent.querySelector("#buffer")) {
          ...
          const cellSize = Math.min(
            (width - minOffset * 5) / size,
            (height - minOffset * 5) / size
          );
        ...
        const renderCarbon = (ctx, pos, carbon, maxCarbon, scaleFactor, rotate) => {
          ...
          dy += (ds - ds * scale) / 2;
          ds *= scale;
          const carbon_png = createImage('carbon', 'src/carbon.svg');
        ...
        if (data(bgCanvas, "step") !== step) {
          ...
          let ChessboardHeight = (botRightCell.dy + botRightCell.ds - topLeftCell.dy + 2 * boxPadding) / size // 棋盘格子高度度

          const chessboard = createImage('chessboard', 'src/chessboard.svg');
          ...
          carbon.forEach((cellCarbon, pos) => renderCarbon(bgCtx, pos, cellCarbon, 50, 1, true));
        }
        ...
        players.forEach((player, playerIndex) => {
          let tree, img
          if (playerIndex == 0) {
            tree = createImage('blue_tree', 'blue_tree.svg');
            img = document.getElementById("blue_tree");
          } else {
            tree = createImage('red_tree', 'red_tree.svg'); 
            img = document.getElementById("red_tree");
          }
          Object.values(player[3]).forEach(pos => {

            const { dx, dy, ds } = getCoords(pos);
            fgCtx.drawImage(img, dx, dy, ds, ds);
          })
        })
        ...
        players.forEach((player, playerIndex) => {
          Object.entries(player[2]).forEach(([uid, [pos, cargo]]) => {
            ...
            renderCarbon(fgCtx, pos, cargo, 50, 0.6, false);
          });
        });
