...
async function renderer({
  ...
  const getCarbon = player => player[0];
  ...
  const writeScoreboardText = (ctx, player, x, y) => {
    ctx.fillText(`Carbon: ${getCarbon(player)}`, x, y);
  ...
  const drawContent = (ctx, x, y, w, h, flag, info) => {
    const textSpacing = 22; // 文字间距
    const upFontPadding = 30; // 文字的上边距(32)
    const leftFontPadding = 16; // 文字的左边距
    const fontSize = 14; // 字体大小

    const img = new Image();
    if (flag === 'left') {
      img.src = './src/left_info.svg';
    } else if (flag === 'right') {
      img.src = './src/right_info.svg';
    }
    ...
    img.onload = function () {
      ...
      if (flag === 'left') {
        ctx.fillText(`资金：${info.fund}万`, x + leftFontPadding, y + upFontPadding);
        ...
        ctx.fillText(`在途CO2数量：${info.CO2num}万吨`, x + leftFontPadding, y + upFontPadding + 4 * textSpacing);
      } else if (flag === 'right') {
        ctx.save();
        ctx.textAlign = 'right';
        ctx.fillText(`资金：${info.fund}万`, x + w - leftFontPadding, y + upFontPadding);
        ...
        ctx.fillText(`捕集CO2数量：${info.CO2num}万吨`, x + w - leftFontPadding, y + upFontPadding + 4 * textSpacing);
        ctx.restore();
      }
    };
  }
  ...
  const drawLogo = (ctx, x, y, w, flag) => {
    const img = new Image();
    if (flag === 'left') {
      img.src = './src/left_avatar.svg';
    } else if (flag === 'right') {
      img.src = './src/right_avatar.svg';
    }
  ...
  const drawTeam = (ctx, x, y, w, h, flag, name) => {
    const upFontPadding = 20; // 文字的上边距(13)
    const leftFontPadding = 45; // 文字的左边距
    const fontSize = 26; // 字体大小

    const img = new Image();
    if (flag === 'left') {
      img.src = './src/left_name.svg';
    } else if (flag === 'right') {
      img.src = './src/right_name.svg';
    }
    ...
    img.onload = function () {
      ...
      ctx.font = `normal ${fontSize}px sans-serif`;
      if (flag === 'left') {
        ctx.fillText(`${name}`, x + leftFontPadding, y + upFontPadding);
      } else if (flag === 'right') {
        ctx.save();
        ctx.textAlign = 'right';
        ctx.fillText(`${name}`, x + w - leftFontPadding, y + upFontPadding);
        ctx.restore();
      }
    }
  }

  const getSymCoord = (x, w = width) => w - x;

  // 绘制队伍信息
  const drawTeamInfo = () => {
    const tWidth = 1440; // 切图布局宽度
    const tHeight = 1024; // 切图布局高度

    const tx = 32; // 左侧队伍名称框起始位置横坐标
    const ty = 364; // 队伍名称框起始位置纵坐标(564)
    const tw = 160; // 队伍名称框宽度
    const th = 63; // 队伍名称框高度

    const cx = 48; // 左侧内容框起始位置横坐标
    const cy = 412; // 内容框起始位置纵坐标(612)
    const cw = 202; // 内容框宽度
    const ch = 143; // 内容框高度

    const lx = 56; // 左侧头像框起始位置横坐标
    const ly = 266; // 头像框起始位置纵坐标(466)
    const lw = 82; // 头像框宽度
    ...
    players.forEach((player, playerIndex) => {
      if (playerIndex === 0) {
        ...
        drawContent(fgCtx, getSymCoord(cx) - cw, cy, cw, ch, 'right', info);
        drawLogo(fgCtx, getSymCoord(lx) - lw, ly, lw, 'right');
        drawTeam(fgCtx, getSymCoord(tx) - tw, ty, tw, th, 'right', '队伍B');
      }
    });
  }
