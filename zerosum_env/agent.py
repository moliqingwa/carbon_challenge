...
class Agent:
    ...
    def act(self, observation):
        ...
        # Start the timer.
        with StringIO() as out_buffer, StringIO() as err_buffer:
            with redirect_stdout(out_buffer) if not self.debug else Elegant(), redirect_stderr(err_buffer)if not self.debug else Elegant():
                try:
                    start = perf_counter()
        ...
        '''
        if duration > self.configuration.actTimeout:
            action = DeadlineExceeded()
        '''
